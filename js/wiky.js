...
var Wiky = {
  ...
  rules: {
     ...
     wikiinlines: [
       ...
       { rex:/\[(?:\{([^}]*)\})?[Ii]ma?ge?\:([^ ,\]]*)(?:[, ]([^\]]*))?\]/g, tmplt:function($0,$1,$2,$3){
       	var rc = "<figure><img"+Wiky.style($1)+" src=\""+$2+"\" alt=\""+($3?$3:$2)+"\" title=\""+($3?$3:$2)+"\"/>";
       	if($3 && $3 != $2){
       		rc += "<figcaption>"+($3)+"</figcaption>"
       	}
       	rc += "</figure>"; 
       	return Wiky.store(rc);} },  // wikimedia image style ..
       { rex:/\[([^ ,]+)[, ]([^\]]*)\]/g, tmplt:function($0,$1,$2){
       	rc = "<a href=\""+$1+"\">"+$2+"</a>";
       	if($1.substr(0,6) == 'mailto'){
       		rc = "<script>document.write(\"<a href=\\\"\"+\"mai\"+\"lto:" + $1.substr(7, 4) + "\"+\"" + $1.substr(11) + "\\\">" + 
       		$2.substr(0,4) + "\"+\"" + $2.substr(4) + "</a>\");</script>";
       	}
       	return Wiky.store(rc);
       	}
       	},  // wiki block style uri's ..
       { rex:/(((http(s?))\:\/\/)?[A-Za-z0-9\._\/~\-:]+\.(?:png|jpg|jpeg|gif|bmp))/g, tmplt:function($0,$1,$2){return Wiky.store("<img src=\""+$1+"\" alt=\""+$1+"\"/>");} },  // simple images .. 
       { rex:/((mailto\:|javascript\:|(news|file|(ht|f)tp(s?))\:\/\/)[A-Za-z0-9\.:_\/~%\-+&#?!=()@\x80-\xB5\xB7\xFF]+)/g, tmplt:"<a href=\"$1\" target=\"blank\">$1</a>" }  // simple uri's .. 
     ],
     escapes: [
   ...
   inverse: {
     ...
     wikiblocks: [
       ...
       { rex:/<BR>/mgi, tmplt:"\n" },
       { rex:/<figure>(.*?)<\/figure>/mgi, tmplt:"$1" },
       { rex:/<figcaption>(.*?)<\/figcaption>/mgi, tmplt:"" },
       { rex:/<script>(.*?)<\/script>/mgi, tmplt:"" },
       { rex:/<h[1-6]>(.*?)<\/h[1-6]>/mgi, tmplt:function($0,$1,$2,$3){
       		var nEquals = parseInt($3.charAt($2 + 2));
       		var sEquals = new Array(nEquals+1).join('=');
       		var rc = sEquals + $1 + sEquals;
       		if($2 != 0 && $3.charCodeAt($2 -1) != 182){
       			rc = "\n" + rc;
       		}
       		if($3.charCodeAt($2 + $0.length) != 182){
       			rc += "\n";
       		}
       		return rc;
       		} },
       ...
       { rex:/<li class=\"?([^ >\"]*)\"?[^>]*?>([^<]*)/mgi, tmplt:function($0,$1,$2,$3,$4){
       		rc = $1.replace(/u/g,"*").replace(/([01aAiIg])$/,"$1.")+" "+$2;
       		if($2 != 0 && $4.charCodeAt($3 -1) != 182){
       			rc = "\n" + rc;
       		}       		
       		return rc;}},  // list items ..
       ...
       { rex:/<\/tr>/mgi, tmplt:function($0,$1,$2){
       		var rc = "|";
       		var nTable = $2.indexOf('table', $1 + 5);
       		var nRow = $2.indexOf('tr', $1 + 5);
       		if($2.charCodeAt($1 + 5) != 182 && !(nRow == -1 || nRow > nTable) ){
       			rc += "\n";
       		}
       		return rc;
       		} },
       ...
       { rex:/<table>/mgi, tmplt:"[" },
       { rex:/<\/table>/mgi, tmplt:"]" },
       { rex:/<tr>\xB6*|<\/td>\xB6*|<tbody>\xB6*|<\/tbody>/mgi, tmplt:function($0,$1,$2){
       		return "";
       		} },
       { rex:/<hr\/?>/mgi, tmplt:"----" },
       { rex:/<br\/?>/mgi, tmplt:"\\\\" },
     ...
     wikiinlines: [
       ...
       { rex:/<img([^>]*)>/mgi, tmplt:function($0,$1){
       	var a=Wiky.attrVal($1,"alt"),h=Wiky.attrVal($1,"src"),t=Wiky.attrVal($1,"title"),s=Wiky.attrVal($1,"style");
       	var rc = h;
       	if( s||(t&&h!=t)){
       		rc = "["+Wiky.invStyle($1)+"img:"+h;
       		if(h!=t){
       			rc += ","+t;
       		}
       		rc += "]";
